 ...
 * BArrayNSet
 * 
 * Implementation of NSet that uses an array of booleans to represent the set.
 * That is, i is in the set iff the array at position i is true.
 * 
...
public class BArrayNSet implements NSet {
	...
	/**
	 * The internal representation of this set, such that i is in the set iff
	 * internal[i].
	 */
	 ...
	 * 
	 * @param range
	 *            One greater than the largest value that can be stored.
	 */
	...
	private void checkParameter(NSet other) {
		if (!(other instanceof BArrayNSet) || other.range() != internal.length)
			throw new BadNSetParameterException(
					this.getClass() + "," + internal.length + " / " + other.getClass() + "," + other.range());
	}
